...
const DiscoveryPhaseIncludes = () => {
  ...
  return (
    <motion.div
      ...
      {/* top section  */}
      <div
        className={`bg-${styles.blackPrimary} text-white xl:w-1/2 lg:w-1/2 w-full flex items-center justify-center flex-col h-full py-10`}>
        {/* Text intro  */}
        <div className="flex gap-5  flex-col xl:w-4/6 lg:w-4/6 w-5/6">
          ...
          <motion.div
            ...
            The discovery phase's main goal is to exclude unknowns, prepare technical basis and UX
            design following your needs, and provide an accurate project time and cost estimation in
            the end.
          </motion.div>
                ...
                <motion.button animate={rightAnimation} key={index}>
                  <div
                    className={`flex flex-row items-center text-${
                      styles.redPrimary
                    } gap-2 text-sm font-semibold py-3 px-2 rounded-3xl drop-shadow-md z-50 w-full ${`job-btn ${
      ...
      <div
        className={`flex flex-col xl:gap-10 lg:gap-10 gap-2 bg-${styles.redPrimary} bg-opacity-90 text-white xl:w-1/2 lg:w-1/2 w-full xl:px-20 lg:px-14 px-10 h-full justify-center py-10`}>
        {/* Show Data  */}
